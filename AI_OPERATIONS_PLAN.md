# AI Operations Plan — agy-software-2
...
| Code host & gate | **GitHub** (`TBD/agy-software-2`) with branch protection + **claude-code-action v1 (GA)** for `@claude` mentions and CI autofix | PRs are the unit of review; CI is the objective referee; `@claude` lets the operator turn a plain-English comment into a fix without opening anything else. |
| App hosting / QA surface | **Fly.io** (Docker container deploy per `fly.toml`) — automatic preview deployment per PR + persistent staging deployment from `develop` | A non-technical human QAs by clicking a URL, never by running anything. |
| Database | **SQLite** (via Prisma; persisted on a Fly.io volume per `fly.toml`) — one production project + one development/staging project; migrations applied by agents via CLI in CI | Keeps destructive experimentation off production. Ephemeral database preview branches (or schema-isolated test databases) isolate e2e runs from the shared dev project (§7.1). |
...
- **Allow:** `npm` scripts, `Node.js`, `tsc`, `Biome`, `Vitest`, `Prisma` CLI (dev instances only), `git` (add/commit/push/branch/fetch/pull), `gh pr` equivalents via the GitHub integration, file edits repo-wide.
