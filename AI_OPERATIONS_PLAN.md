# AI Operations Plan — agy-software-2 (AnswerFlow AI)
...
| Code host & gate | **GitHub** (`michaelcrosato/agy-software-2`) with branch protection + **claude-code-action v1 (GA)** for `@claude` mentions and CI autofix | PRs are the unit of review; CI is the objective referee; `@claude` lets the operator turn a plain-English comment into a fix without opening anything else. |
...
| Database | **SQLite via Prisma** (local-first is the product's identity) — local file in dev/tests; staging persistence on the QA surface is an open Phase 1 design point (roadmap/DECISIONS.md 2026-06-10); migrations applied by agents via Prisma CLI in CI | Keeps destructive experimentation off production. Ephemeral database preview branches (or schema-isolated test databases) isolate e2e runs from the shared dev project (§7.1). |
...
- **Allow:** `npm` scripts, `node`, `tsc`, `biome`/`eslint`, `vitest`/`playwright`, `prisma` CLI (dev instances only), `git` (add/commit/push/branch/fetch/pull), `gh pr` equivalents via the GitHub integration, file edits repo-wide.
